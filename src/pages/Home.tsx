...
export default function Home() {
  ...
  return (
    <div className={`mx-auto px-4 py-3 sm:py-4 ${twoCol ? 'max-w-none h-full min-h-0 flex flex-col' : 'max-w-5xl space-y-3 sm:space-y-4'}`}>
      {twoCol ? (
        /* ===== 桌面「全部功能」模式：A1 左右分栏，一屏显示 ===== */
        <div className="grid grid-cols-[1.35fr_1.3fr] gap-6 h-full min-h-0 items-stretch overflow-hidden">
          {/* 左列：信息区（压缩至一屏，不出现可见滚动条；超高时仍可隐藏滚动） */}
          <div className="flex flex-col gap-3 min-h-0 overflow-y-auto overscroll-contain pr-1 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
            {/* 打卡提醒 */}
            ...
            {/* 倒计时 */}
            <div className="rounded-xl bg-white dark:bg-slate-900 border border-gray-100 dark:border-slate-800 px-3 py-2">
